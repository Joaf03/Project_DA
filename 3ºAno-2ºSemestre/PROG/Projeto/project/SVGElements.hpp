...
namespace svg
{
    ...
    {
    public:
        line(const Point &start, const Point &end, const Color &fill) : polyline(fill, std::vector<Point>{start, end}), start(start), end(end) {}
    ...
    {
    public:
        polygon(const Color &fill, const std::vector<Point> &points) : fill(fill), points(points) {}
        void draw(PNGImage &img) const override {}
        void translate(float dx, float dy) override;
        void rotate(float angle, const Point &center) override;
        void scale(float sx, float sy) override;
    ...
    {
    public:
        rect(const Color &fill, const Point &upper_left_corner, const int &width, const int &height) : polygon(fill, std::vector<Point>{upper_left_corner, {upper_left_corner.x + width, upper_left_corner.y}, {upper_left_corner.x + width, upper_left_corner.y + height}, {upper_left_corner.x, upper_left_corner.y + height}}),
